"""
Interactive chat with GAI v8.4 (prefixed dialog mode — best model).
Uses > prefix to maintain dialog mode.
"""
import sys, os, torch
...
MODEL_PATH = "gai_model_v8_4_max.pkl"
TEMPERATURE = 0.6
TOP_K = 30
MAX_NEW = 200
SEQ_LEN = 256
...
print("Loading GAI v8.4...")
model, tokenizer, config = GPT.load_pickle(MODEL_PATH)
model.eval()
safe_print(f"Model: {sum(p.numel() for p in model.parameters()):,} params, vocab={len(tokenizer)}")
safe_print("\n=== GAI v8.4 ===")
